...
class RealTradingExecutor {
    ...
    async executeTradeForSingleUser(user, signalData, signalId) {
        const userResult = {
            username: user.username,
            successful_executions: 0,
            failed_executions: 0,
        ...
        try {
            ...
            const leverage = signalData.leverage || user.leverage || 1;
            const maxLeverage = this.tradingConfig.max_leverage;
            
            if (leverage > maxLeverage) {
                console.log(`  ❌ Leverage ${leverage}x excede o máximo permitido (${maxLeverage}x)`);
                return userResult;
            }

            // Verificar se stop loss é obrigatório
            const stopLoss = signalData.stopLoss || user.stopLoss;
            if (this.tradingConfig.mandatory_stop_loss && !stopLoss) {
                console.log('  ❌ Stop Loss é obrigatório para este usuário');
                return userResult;
            }
            ...
            if (positionValue > this.tradingConfig.max_position_size) {
                console.log(`  ❌ Valor da posição ($${positionValue}) excede o máximo permitido`);
                return userResult;
            }
