...
class CorrecaoDependenciasBanco {
    ...
    async corrigirConnectionStrings(nomeArquivo) {
        ...
        try {
            ...
            const connectionStringHardcoded = 'process.env.DATABASE_URL';
    ...
    ssl: { rejectUnauthorized: false }
            ...
            if (conteudo.includes('process.env.DATABASE_URL"process.env.DATABASE_URL"require('dotenv').config()")) {
        ...
        try {
            ...
            if (conteudo.includes('new Pool({') && conteudo.includes('.query(')) {
                const temConnectionString = conteudo.includes('connectionString:') || 
                                          conteudo.includes('DATABASE_URL"process.env.DATABASE_URL"process.env.DATABASE_URL',
    ssl: { rejectUnauthorized: false }
})`
                    );
            ...
            if (conteudo.includes('.query(') && !conteudo.includes('Pool') && !conteudo.includes('this.pool')) {
                ...
                if (!conteudo.includes("require('pg')")) {
                    ...
                    if (primeiraLinha >= 0) {
                        ...
                        linhas.splice(primeiraLinha + 4, 0, "    connectionString: process.env.DATABASE_URL || 'process.env.DATABASE_URL',");
