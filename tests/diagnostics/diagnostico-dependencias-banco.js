...
class DiagnosticoDependenciasBanco {
    ...
    async analisarArquivo(nomeArquivo) {
        ...
        try {
            ...
            if (conteudo.includes('DATABASE_URL"process.env.DATABASE_URL"process.env.DATABASE_URL"process.env.DATABASE_URL".query(') || conteudo.includes('pool.query') || conteudo.includes('client.query')) {
            ...
            if (conteudo.includes('PROTECTED_DB_PASSWORD') && !conteudo.includes('process.env.DATABASE_URL"process.env.DATABASE_URL"SEGURANÇA: Connection string hardcoded sem fallback de env');
    ...
    async gerarRelatorio() {
        ...
        if (this.connectionStringsEncontradas.size === 0) {
            ...
            this.connectionStringsEncontradas.forEach(conn => {
                ...
                } else if (conn.includes('DATABASE_URL"process.env.DATABASE_URL"app.js', 'financial-manager.js', 'commission-system.js', 'dashboard-real-final.js'];
